...
<div class="ml-64 p-8 bg-gray-100 min-h-screen transition-all duration-500" id="main-content">
  ...
  <h1 class="text-3xl font-bold mb-2">Hi, <?= $result['first_name']?></h1>
    ...
    <div class="absolute right-10 top-10 bg-orange-500 text-white rounded-lg px-4 py-2 flex items-center space-x-2 text-lg shadow-lg hover:bg-orange-600">
      <a href="logic/create.php" class="flex items-center space-x-2">
      ...
      <?php foreach ($venues as $venues): ?>
        <div class="flex flex-col justify-center items-center relative bg-white shadow-lg border border-gray-300 rounded-lg p-6 hover:shadow-xl hover:scale-105 transition-transform duration-300 ease-in-out w-full sm:w-[calc(33%-20px)] h-80">
          ...
          <div class="absolute top-4 right-6 flex space-x-3">
            <a href="logic/update.php?id=<?php echo $venues['idvenues']; ?>" class="text-gray-600 hover:text-gray-800">
              <span>Edit</span>
          </a>
          <a>
              <span>|</span>
            ...
            <a href="logic/delete.php?id=<?php echo $venues['idvenues']; ?>" class="text-gray-600 hover:text-gray-800" onclick="return confirm('Are you sure you want to delete this venue?')">
              <span>Delete</span>
            </a>
          </div>
        </div>
